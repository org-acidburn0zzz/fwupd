...
static gboolean
fu_rts_usbhub_rtd21xx_device_reload (FuDevice *device, GError **error)
{
    ...
    if (locker == NULL)
        return FALSE;
    return fu_rts54hub_module_setup (device, error);
}

static gboolean
fu_rts_usbhub_rtd21xx_device_detach (FuDevice *device, GError **error)
{
    FuRts54HubDevice *parent = FU_RTS54HUB_DEVICE (fu_device_get_parent (device));
    g_autoptr(FuDeviceLocker) locker = NULL;
    /* open device */
    ...
    if (!fu_device_retry (device,
                  fu_rts_usbhub_device_rtd21xx_detach_cb,
                  100, NULL, error))
        return FALSE;
...
static gboolean
fu_rts_usbhub_rtd21xx_device_attach (FuDevice *device, GError **error)
{
    FuRtsUsbhubRtd21xxDevice *self = FU_RTS_USBHUB_RTD21XX_DEVICE (device);
    FuRts54HubDevice *parent = FU_RTS54HUB_DEVICE (fu_device_get_parent (device));
...
fu_rts_usbhub_rtd21xx_device_class_init (FuRtsUsbhubRtd21xxDeviceClass *klass)
{
    ...
    klass_device->reload = fu_rts_usbhub_rtd21xx_device_reload;
    klass_device->attach = fu_rts_usbhub_rtd21xx_device_attach;
    klass_device->detach = fu_rts_usbhub_rtd21xx_device_detach;
